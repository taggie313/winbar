...
@Suite struct CreateScripting {
    ...
    @Test func mapsTheScriptsErrorNumbers() {
        ...
        #expect(failure("-:1346:1413: execution error: The VM must be shut down before its install disks can be detached. (1112)")
                == .wrongState("The VM must be shut down before its install disks can be detached."))
        ...
        #expect(failure("-:4490:4544: execution error: After detaching the install disks UTM reports: the display count changed (1114)")
                == .changedUnexpectedly("After detaching the install disks UTM reports: the display count changed"))
